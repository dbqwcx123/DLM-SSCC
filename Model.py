...
import copy
import logging
from utils.ECCT_utils import sign_to_bin, bin_to_sign
...
class ECC_Transformer(nn.Module):
    def __init__(self, args, dropout=0):
        ...
        #############
        self.src_embed = nn.Parameter(torch.empty(
            (self.code.n + self.code.pc_matrix.size(0), args.d_model)))  # shape (2n-k, d)
        #############
        # MLP (with self-attention block and feed-forward networks)
        self.decoder = Encoder(EncoderLayer(
            args.d_model, copy.deepcopy(attn), copy.deepcopy(ff), dropout), args.N_dec)
        self.oned_final_embed = torch.nn.Sequential(
            *[nn.Linear(args.d_model, 1)])
        self.out_fc = nn.Linear(self.code.n + self.code.pc_matrix.size(0), self.code.n)

        self.get_mask(self.code)
        logging.info(f'Mask:\n {self.src_mask}')
        ###
    ...
    def forward(self, magnitude, syndrome):
        ...
        emb = self.src_embed.unsqueeze(0) * emb  # High dimension embedding
        output = self.decoder(emb, self.src_mask)
        z_pred = self.out_fc(self.oned_final_embed(output).squeeze(-1))
        return z_pred

    def loss(self, z_pred, z_mul, y):
        loss = F.binary_cross_entropy_with_logits(
            -z_pred, sign_to_bin(torch.sign(z_mul)))
    ...
    def get_mask(self, code, no_mask=False):
        ...
        a = mask_size ** 2
        logging.info(
            f'Self-Attention Sparsity Ratio={100 * torch.sum((src_mask).int()) / a:0.2f}%, \
              Self-Attention Complexity Ratio={100 * torch.sum((~src_mask).int())//2 / a:0.2f}%')
