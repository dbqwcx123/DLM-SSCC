...
def Get_Generator_and_Parity(code, standard_form = False):
    ...
    if code.code_type in ['POLAR']:
        ParityMatrix = np.loadtxt(path_pc_mat+'.txt')
        pc_matrix_I, p = row_reduce(ParityMatrix.astype(bool), ncols=ParityMatrix.shape[1])
        ParityMatrix2 = pc_matrix_I.astype(int)
    elif code.code_type in ['BCH', 'LTE_TURBO']:
        ParityMatrix = np.loadtxt(path_pc_mat+'.txt')
        pc_matrix_I, p = row_reduce(ParityMatrix.astype(bool), ncols=ParityMatrix.shape[1])
        ParityMatrix2 = pc_matrix_I.astype(int)
    ...
    if standard_form and code.code_type not in ['CCSDS', 'LDPC', 'MACKAY']:
        ...
        ParityMatrix2 = get_standard_form(ParityMatrix2).astype(int)
        GeneratorMatrix = np.concatenate([np.mod(-ParityMatrix[:, min(ParityMatrix.shape):].transpose(),2),np.eye(k)],1).astype(int)
    else:
        GeneratorMatrix = get_generator(ParityMatrix2)
        # np.savetxt('G.txt',GeneratorMatrix,fmt='%d', delimiter=',')
        # np.savetxt('H.txt',ParityMatrix,fmt='%d', delimiter=',')
        # np.savetxt('H_sys.txt',ParityMatrix2,fmt='%d', delimiter=',')
    assert np.all(np.mod((np.matmul(GeneratorMatrix, ParityMatrix2.transpose())), 2) == 0) and np.sum(GeneratorMatrix) > 0
    return GeneratorMatrix.astype(float), ParityMatrix2.astype(float), ParityMatrix.astype(float)
