...
class DiffusionArguments:
    ...
    score_temp: float = field(
        default=0.5,
        metadata={"help": "soft the pred epsilon"}
    )
    logits_temp: float = field(
        default=1.0,
        metadata={"help": "soft the pred logits"}
    )
    shift: bool = field(
        default=False,
        metadata={"help": "shift prediction or not"}
    )
    anneal_steps: int = field(
        default=10000,
        metadata={"help": "training steps of mask annealing."}
    )

@dataclass
class FinetuningArguments(FreezeArguments, LoraArguments, RLHFArguments, GaloreArguments, BAdamArgument, DiffusionArguments):
    r"""
    ...
    stage: Literal["pt", "sft", "rm", "ppo", "dpo", "kto", "ddm", "ddm-sft"] = field(
        default="sft",
