...
def get_args():
    ...
    parser.add_argument("--diffu_steps", type=int, default=20)
    ...
    # clean : 完全干净图像输入，不做 MASK，直接可视化真实图像上下文下的置信度
    parser.add_argument(
        "--visualize_mode",
        type=str,
        default="clean",
        choices=["init", "mid", "final", "custom", "clean"]
    )
    parser.add_argument(
        "--rollout_until_t",
        type=int,
    ...
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default="../Model/diffugpt-s/ddm-sft/train_20251226_231454/checkpoint-26000"
    )
    # parser.add_argument(
    #     "--checkpoint_path",
    #     type=str,
    #     default="../Model/diffugpt-s/ddm-sft/train_full_20260415_021825/checkpoint-31000"
    # )
...
def get_stop_t(args):
    ...
    if args.visualize_mode == "clean":
        # clean 模式不需要 rollout，返回 None 作为标记
        return None
...
def rollout_to_state(
    ...
    with torch.no_grad():
        for t in range(diffu_steps - 1, stop_t - 1, -1):
            ...
            # 只允许仍然 masked 的位置参与排序。
            # 已经 reveal 的位置不应该再次被选择。
            confidences = confidences.masked_fill(~maskable_mask, float("-inf"))

            # 轻微量化，减少极小浮点差异带来的排序不稳定。
...
def main(args):
    ...
    if args.visualize_mode == "clean":
        print("⚡ 可视化模式: clean（完全干净图像输入，无 MASK）")
    elif args.visualize_mode == "init":
    ...
    with torch.inference_mode():
        for i in tqdm(range(0, len(patches_flat), batch_size)):
            ...
            if args.visualize_mode == "clean":
                # clean 模式：
                # 直接输入完整真实图像 token，不做 MASK，不做 rollout。
                # 此时可视化的是模型在完全干净上下文下，对每个像素位置的预测置信度/熵。
                xt = x_true_ids

                # clean 模式没有 masked 位置。
                # 为了兼容后续空间重组，这里构造全 0 mask map。
                maskable_mask = torch.zeros_like(x_true_ids, dtype=torch.bool, device=device)

            else:
                # rollout 模式：
                # 从全 MASK 状态按置信度逐步 reveal 到目标状态。
    ...
    all_patch_confs = np.concatenate(all_patch_confs, axis=0)
    all_patch_mask_states = np.concatenate(all_patch_mask_states, axis=0)

    # [Nh*Nw, patch_h*patch_w*C] -> [Nh, Nw, patch_h, patch_w, C]
    ...
    # RGB 三通道取均值，得到空间 heatmap
    conf_spatial = conf_patches.mean(axis=-1)
    mask_spatial = mask_patches.mean(axis=-1)

    heatmap = conf_spatial.transpose(0, 2, 1, 3).reshape(new_H, new_W)
    maskmap = mask_spatial.transpose(0, 2, 1, 3).reshape(new_H, new_W)

    # rollout 模式下，只看当前仍然 masked 的位置更有解释性。
    # clean 模式下没有 masked 位置，因此直接展示整张图的置信度/熵。
    if args.visualize_mode == "clean":
        display_heatmap = heatmap
        state_title = "Clean Input State (0=No Mask)"
        conf_title = "Confidence / Entropy on Clean Image"
    else:
        display_heatmap = heatmap.copy()
        display_heatmap[maskmap < 0.5] = np.nan
        state_title = f"Current Mask State (1=Masked, t={stop_t})"
        conf_title = f"Confidence on Remaining Masks (t={stop_t})"
    ...
    plt.subplot(1, 3, 2)
    plt.title(state_title, fontsize=14)
    plt.imshow(maskmap, cmap="gray", vmin=0.0, vmax=1.0)
    plt.axis("off")

    plt.subplot(1, 3, 3)
    plt.title(conf_title, fontsize=14)
    im = plt.imshow(display_heatmap, cmap="magma")
